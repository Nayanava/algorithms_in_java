...
public class CherryPickup {
    public int cherryPickup(int[][] grid) {
        ...
        for(int i = 1; i < n; i++) {
            if(grid[i][0] != -1 && (i == 1 || dp[i-1][0].count != 0)) {
        ...
        for(int j = 1; j < n; j++) {
            if(grid[0][j] != -1 && (j == 1 || dp[0][j-1].count != 0)) {
        ...
        for(int i = 1; i < n; i++) {
            for(int j = 1; j < n; j++) {
                if(grid[i][j] == -1 || (dp[i-1][j].count == 0 && dp[i][j-1].count == 0)) {
                    dp[i][j].count = 0;
                    continue;
                }
        ...
        for(i = n-2; i >= 0; i--) {
            if(grid[i][0] != -1 && (i == n-2 || resDP[i+1][0] != 0)) {
                resDP[i][0] = grid[i][0] + resDP[i+1][0];
            }
        }
        for(j = n-2; j >= 0; j--) {
            if(grid[0][j] != -1 && (j == n-2 || resDP[0][j+1] != 0)) {
        ...
        for(i = n-2; i >= 0; i--) {
            for(j = n-2; j >= 0; j--) {
                if(grid[i][j] != -1 && (resDP[i+1][j] != 0 && resDP[i][j+1] != 0)) {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(cp.cherryPickup(dp));
    }
}
